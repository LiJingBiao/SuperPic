...
- (instancetype)initShow:(UIViewController *)handleVC index:(NSUInteger)index pageCount:(NSUInteger)pageCount
{
    self = [super init];
    if (self) {
...
-(void)zoomPhotoVC{
    ...
    PhotoModel *photoModel = [self.delegate getPhotoModelFromIndex:self.index];
...
-(void)pagesPrepare{
    ...
    //设置contentSize
    NSUInteger totalCount = [self.delegate totalShowImageCount];
    self.scrollView.contentSize = CGSizeMake(widthEachPage * totalCount, 0);
...
-(void)showWithPage:(NSUInteger)page{
    ...
    photoItemView.photoModel = [self.delegate getPhotoModelFromIndex:page];
...
- (IBAction)rightBtnClick:(id)sender {
    ...
    PhotoModel *itemModel = [self.delegate getPhotoModelFromIndex:self.page];
...
-(void)zoomOut{
    ...
    PhotoModel *photoModel = [self.delegate getPhotoModelFromIndex:self.page];
